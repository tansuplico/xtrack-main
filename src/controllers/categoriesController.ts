...
export const fetchCategories = expressAsyncHandler(
...
export const modifyCategories = expressAsyncHandler(
  async (req: Request, res: Response) => {
    // @ts-ignore
    const user = req.user;
    ...
    if (type === "Expenses") {
      ...
      const similarExp = userCategory?.expenses.find(
        (d) => d.category === newName
      );
      const similarInc = userCategory?.income.find(
        (d) => d.category === newName
      );
...
export const createCategories = expressAsyncHandler(
  async (req: Request, res: Response) => {
    // @ts-ignore
    const user = req.user;
...
export const deleteCategories = expressAsyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (type === "Expenses") {
      ...
      if (!updated) {
        res.status(400).json({ error: "An error has occured while deleting" });
        return;
      }
    } else if (type === "Income") {
      const updated = await Categories.findOneAndUpdate(
