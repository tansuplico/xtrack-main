...
export const fetchTransactions = expressAsyncHandler(async (req, res) => {
    try {
        // @ts-ignore
        const user = req.user;
...
export const addTransactions = expressAsyncHandler(async (req, res) => {
    try {
        // @ts-ignore
        const user = req.user;
        const { data } = req.body;
        ...
        const updatedTransaction = await Transactions.findOneAndUpdate({ id: user.id }, {
            $push: { history: data },
        }, { new: true });
        if (data.categoryData.type === "Expenses") {
            await Categories.updateOne({ id: user.id, "expenses.category": data.categoryData.category }, { $inc: { "expenses.$.transactionCount": 1 } }, { new: true });
        }
        else if (data.categoryData.type === "Income") {
            await Categories.updateOne({ id: user.id, "income.category": data.categoryData.category }, { $inc: { "income.$.transactionCount": 1 } }, { new: true });
        }
...
export const clearHistory = expressAsyncHandler(async (req, res) => {
    try {
        // @ts-ignore
        const user = req.user;
        const updatedTransaction = await Transactions.findOneAndUpdate({ id: user.id }, { $set: { history: [] } }, { new: true });
        const updatedWallet = await Wallet.findOneAndUpdate({ id: user.id }, { $set: { expense: 0 } }, { new: true });
        await Categories.updateOne({ id: user.id }, {
            $set: {
...
export const removeTransactions = expressAsyncHandler(async (req, res) => {
    try {
        // @ts-ignore
        const user = req.user;
        ...
        const transactionToRemove = transaction.history.find((tr) => String(tr._id) === id);
        if (type === "Expenses") {
            await Categories.updateOne({ id: user.id, "expenses.category": category }, {
                $inc: { "expenses.$.transactionCount": -1 },
            }, { new: true });
        }
        else if (type === "Income") {
            await Categories.updateOne({ id: user.id, "income.category": category }, {
        ...
        const updatedTransaction = await Transactions.findOneAndUpdate({ id: user.id }, { $pull: { history: { _id: id } } }, { new: true });
        if (!updatedTransaction) {
